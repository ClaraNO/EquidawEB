...
public class VenteDAO {
    ...
    */
    ...
    // nouveau 
    // Changer idVente en String
    public static ArrayList<Mail>  getLesMails(Connection connection, String VenteId){      
       ArrayList<Mail> lesMails = new  ArrayList<Mail>();
        try
        {
            //preparation de la requete     
            //codeCateg="ETE";
            requete=connection.prepareStatement("SELECT mail.* FROM vente, mail WHERE vente.id = mail.VenteId AND vente.id = ? ");
            // Changer idVente en String
            requete.setString(1, VenteId);
            //executer la requete
            rs=requete.executeQuery();
            ...
            while ( rs.next() ) {  
                ...
                unMail.setDate(rs.getInt("date"));
                unMail.setObjet(rs.getString("objet"));
                unMail.setCorps(rs.getString("corps"));
                
                
                //unMail.setUneVente(v);
                /*CategVente uneCateg = new CategVente();
                uneCateg.setCode(rs.getString("code"));  // on aurait aussi pu prendre CodeCateg
                uneCateg.setLibelle(rs.getString("libelle"));*/
                System.out.println("VENTEDAO" +unMail.toString() );
                lesMails.add(unMail);
            }
        }   
        catch (SQLException e) 
